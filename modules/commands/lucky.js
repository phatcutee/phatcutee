  ...
  if(money < 5) api.sendMessage("Bạn không đủ tiền !",event.threadID,event.messageID)
      ...
      if(!args[0]) api.sendMessage("Không có số dự đoán.",event.threadID,event.messageID)
         ...
         if(args[0] > 5) api.sendMessage("Dự đoán không được lớn hơn 5.",event.threadID,event.messageID)
             ...
             if(args[0] == number){
                 api.sendMessage(number + " chính là con số may mắn, bạn đã nhận được 500 đô.", event.threadID, () => Currencies.setData(event.senderID, options = {money: money + parseInt(i)}),event.messageID);
                }
         else api.sendMessage("Con số may mắn là " + number + "\n" + "Chúc bạn may mắn lần sau nhaaa !\n====Lưu ý====\nSau mỗi lần đoán sai, bạn sẽ bị trừ 100 đô, nếu bạn đúng bạn sẽ nhận lại 500 đô.",event.threadID, () => Currencies.setData(event.senderID, options = {money: money - 100}),event.messageID);
      }
    }
  }
};