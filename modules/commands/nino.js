...
module.exports.handleEvent = async ({ api, event, args, Threads }) => {
    ...
    if (nino.hasOwnProperty(threadID) && nino[threadID] == true) {
      ...
      axios.get(encodeURI(`https://adreno-api.rootandroid.repl.co/nino/get/${event.body}`)).then(res => {
            if (res.data.reply == "null" || res.data.reply == "ủa nói gì hong hiểu :<") {
                api.sendMessage("nino hok hiểu, dạy nino đi :<",threadID,messageID)
            } else {
...
module.exports.run = async ({ api, event, args, Threads }) => {
    ...
    if (!args[0]) { api.sendMessage("ủa hỏi gì hỏi đi ;-;", threadID, messageID) } else {
        switch(args[0]) {
            case "on": {
                nino[threadID] = true;
                api.sendMessage("đã bật nino thành công", threadID);
                break;
            }
            case "off": {
                nino[threadID] = false;
                api.sendMessage("bái baiii", threadID);
                break;
            }
            default:
